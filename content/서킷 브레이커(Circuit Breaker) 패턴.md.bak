서킷 브레이커(Circuit Breaker) 패턴은 [[분산 시스템(Distributed System)]]에서 장애 전파를 방지하고 시스템의 [[회복력]]을 높이기 위한 디자인 패턴입니다. 이 패턴은 전기 회로의 차단기에서 이름을 따왔으며, 전기 시스템에서 과부하가 발생했을 때 회로를 차단하는 것과 유사한 방식으로 동작합니다.
