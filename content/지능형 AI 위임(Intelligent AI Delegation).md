...
논문은 위임을 분석하기 위한 여러 축(axis)을 정의합니다. 작업을 위임할 때 고려해야 할 작업 특성은 다음과 같습니다:
...
이 외에도 세분성(요청이 얼마나 구체적인가), 자율성(수임자에게 얼마나 자유를 주는가), 모니터링 방식(연속적/주기적/이벤트 기반), 상호성(일방적 요청인가, 상호 위임인가) 등의 축이 존재합니다.
...
논문의 핵심인 프레임워크는 다섯 가지 요구사항을 중심으로 설계됩니다:
...
- 팀 응집력 보존: 인간 간 관계를 약화시키지 않도록 주의
- 심리적 안전(psychological safety) 보호: 프라이버시 존중, 업무 흐름 경계 인식

### 6.5. 사용자 교육

인간 참여자가 에이전트 시스템 내에서 위임자, 수임자, 감독자로 효과적으로 기능하기 위한 전문성을 갖추도록 AI 리터러시 향상이 필요합니다. 기술적 조치는 작업 민감도와 도메인 맥락에 기반한 정책 프레임워크에 의해 강화되어야 합니다.
...
Macaroon/Biscuit 기반의 감쇠된 인가 토큰. 체인의 각 참여자가 후속 제한을 추가하여 범위를 더욱 제한.
