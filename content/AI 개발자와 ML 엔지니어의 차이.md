...
가령, IBM과 같은 글로벌 기업은 AI 개발자를 'AI 모델을 실제 소프트웨어 솔루션에 적용하는 역할'로 정의하는 반면, 일부 채용 공고나 기술 블로그에서는 '모델 설계부터 배포까지 전 과정을 담당하는 역할'로 설명하기도 합니다. 이 글에서는 공신력 있는 기관의 정보를 바탕으로 AI 및 LLM 분야의 주요 직무를 명확히 정의하고자 합니다.
...
```mermaid
graph TD
    subgraph "Phase 1: 연구 및 모델링"
        A["AI/ML 리서처"] -- "신규 알고리즘/모델 연구" --> C
        B["데이터 과학자"] -- "데이터 분석 및 모델 프로토타입 설계" --> C
    end

    subgraph "Phase 2: 시스템 구축 및 배포"
        C["ML/AI 엔지니어"]
        C -- "모델 생명주기 관리<br/>(MLOps 파이프라인 구축 및 운영)" --> D_API["<b>배포된 AI 모델 (API)</b>"]
    end

    subgraph "Phase 3: 응용 및 서비스 개발"
        D_API -- "API 활용" --> E["AI 개발자"]
        F["프롬프트 엔지니어"] -- "최적화된 프롬프트 설계" --> E
        E -- "AI 기능 통합/구현" --> G["사용자 애플리케이션<br/>(웹, 앱 서비스 등)"]
    end
```
---
## 결론 및 시사점
