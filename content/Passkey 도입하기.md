...
실제 프로덕션 환경에서는 이러한 데이터 변환 및 WebAuthn 사양의 복잡한 검증 로직을 직접 구현하기보다는 SimpleWebAuthn과 같은 검증된 라이브러리를 사용하는 것이 강력히 권장됩니다. `SimpleWebAuthn`은 TypeScript 기반으로 프론트엔드(`@simplewebauthn/browser`)와 백엔드(`@simplewebauthn/server`) 모두를 위한 기능을 제공하여 구현을 크게 단순화할 수 있습니다.
